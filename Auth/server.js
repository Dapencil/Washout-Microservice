...
app.post("/user", async (req, res) => {
  ...
  try {
    ...
    const conn = await pool.getConnection();
    const queryString = `INSERT INTO ${USER_TABLE}(uid, branchID, username, password, role, deviceToken) VALUES (DEFAULT,DEFAULT,?, ?, DEFAULT,?)`;
...
app.post("/staffs/", accessTokenValidate, async (req, res) => {
  ...
  try {
    const hashedPassword = await bcrypt.hash(password, SALT_ROUNDS);
    const conn = await pool.getConnection();
    const queryString_addUser = `INSERT INTO ${USER_TABLE}(uid, branchID, username, password, role, deviceToken) VALUES (DEFAULT, ?, ?, ?, "staff", NULL)`;
